...
class Round:
    ...
    def create_random_pairs(self, players):
        ...
        self.matches = []
        print("Les matchs pour ce round sont : \n")
        for pair in pairs:
            player1 = pair[0]
            player2 = pair[1]
            # Crée un objet Match et ajoute à la liste des matches du tour
    ...
    def generate_pairs_for_next_round(self, players, previous_results, sorted_players):
        ...
        sorted_players_info = [
            (player_id, score, Player.get_player_by_id(player_id))
        ...
        for player_id, points, player in sorted_players_info:
            print(
                f"{player.first_name} {player.last_name} {player.player_id}: {points} points"
            )
